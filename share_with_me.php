...
if (isset($_GET['key'])) {
    ...
    // Get key từ url
    $key = $_GET['key'];
    // Lấy cái đặt của file được share có key share trùng với key từ url
    ...
    if ($query) {
        if (mysqli_num_rows($query) > 0) {
            ...
            $us = $d['users'];
            $is_all = $d['isAll'];
            // Kiểm tra file được share cho đối tượng cụ thể hay share toàn bộ ng dùng
            if ($is_all == 1) {
                // Nếu share toàn bộ -> lấy thông tin file 
                $sel_file = "SELECT * FROM file WHERE id='$file_id'";
                $run_sel = mysqli_query($connect, $sel_file);
                // lấy dữ liệu trong table share_with_me để kiểm tra người dùng có từng truy cập vào file chưa
                $sel_exits = "SELECT * FROM share_with_me WHERE id_file='$file_id'";
                $run_ex = mysqli_query($connect, $sel_exits);
                if ($run_ex) {
                    if (mysqli_num_rows($run_ex) == 0) {
                        // Nếu chưa từng truy cập -> thêm file vào table
                ...
                // Nếu là file share cho đối tượng cụ thể
                // Kiểm tra username của user với list user được share
                if (strpos($us, $email) !== false) {
                    // Nếu user là đối tượng dc share=> lấy file 
                    $sel_file = "SELECT * FROM file WHERE id='$file_id'";
                    $run_sel = mysqli_query($connect, $sel_file);
                    $num = mysqli_num_rows($run_sel);
                } else {
    ...
    //  Lấy tất cả file được share với user
    $sql_sl_share = "SELECT * FROM share_with_me,file WHERE share_with_me.username='$email' AND share_with_me.id_file=file.id";
    $run_sel = mysqli_query($connect, $sql_sl_share);
    $num = mysqli_num_rows($run_sel);
}
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="dropdown" id="dropdownThuMuc" style=" background-color: rgb(247, 251, 252);color: rgb(0, 74, 124);font-family: 'Times New Roman', Times, serif;">
                                            ...
                                            <ul class="dropdown-menu">
                                                ...
                                                <li><a class="dropdown-item cursor-pointer" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="getDetail(
